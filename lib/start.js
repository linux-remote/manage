...
const { serverFilePath, confPath, serverMainFilePath, serverUserFilePath } = require('./util/paths.js');
...
function _start(oldPid){
  const subprocess = spawn(process.argv[0], [serverFilePath], {
    env: { // reduce unnecessary copy
      ...
      SERVER_MAIN_PATH: serverMainFilePath,
      SERVER_USER_PATH: serverUserFilePath
    },
    detached: true,
    cwd: homeDir,
